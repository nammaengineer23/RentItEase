...
class _AddPropertyPageState extends ConsumerState<AddPropertyPage> {
  ...
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: Text(context.tr('addProperty'))), body: Form(key: _formKey, child: ListView(padding: const EdgeInsets.all(16), children: [
    ...
    const SizedBox(height:16), const Text('Video tour (optional)',style:TextStyle(fontSize:18,fontWeight:FontWeight.bold)), const Text('One MP4, MOV or M4V video • up to 60 seconds • 100 MB'), OutlinedButton.icon(onPressed:loading?null:_pickVideo,icon:const Icon(Icons.video_call_outlined),label:Text(selectedVideo==null?'Select video tour':'Change selected video',overflow:TextOverflow.ellipsis)), if(selectedVideo!=null) Card(child:ListTile(leading:const Icon(Icons.video_file_outlined),title:Text(selectedVideo!.name,overflow:TextOverflow.ellipsis),subtitle:Text('${(selectedVideo!.size / (1024 * 1024)).toStringAsFixed(1)} MB • Ready to upload when property is created'),trailing:IconButton(onPressed:loading?null:()=>setState(()=>selectedVideo=null),icon:const Icon(Icons.close),tooltip:'Remove selected video'))),
